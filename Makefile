...
roll-cubing: setup
	git pull
	bun x @lgarron-repo/repo dependencies --package-manager bun roll --commit cubing
	git push
	make deploy
...
roll-@cubing/icons:
	git pull
	bun x @lgarron-repo/repo dependencies --package-manager bun roll --commit @cubing/icons
	git push
	make deploy
