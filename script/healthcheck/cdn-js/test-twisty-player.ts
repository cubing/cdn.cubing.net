...
await page.goto(new URL("test-twisty-player.html", import.meta.url).toString());
...
await run("<twisty-player> pattern", async () => {
  ...
  const observedKPattern = new KPattern(await cube3x3x3.kpuzzle(), patternData);
...
await run("<twisty-player> time range", async () => {
  const timeRange: { start: number; end: number } = await page.evaluate(() => {
    return (globalThis as GlobalThis).player.experimentalModel.timeRange.get();
  });
